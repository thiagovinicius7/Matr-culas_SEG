...
export default function StudentProfile({
  ...
  const emProcessoDeMatricula = !!currentYearEnrollment && currentYearEnrollment.faseProcesso !== 'colheita';
                  ...
                  {emProcessoDeMatricula && currentYearEnrollment && (
                    ...
                    >
                      <span className="text-xs font-bold text-brand-clay">
                        {faseLabels[currentYearEnrollment.faseProcesso].emoji} Matrícula {activeYear} em {faseLabels[currentYearEnrollment.faseProcesso].label}
                      </span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs">
                    <div className="flex items-stretch justify-between gap-1">
                      {faseOrderList.map((faseKey, idx) => {
                        const currentIdx = faseOrderList.indexOf(currentYearEnrollment.faseProcesso);
                  ...
                  <div className="bg-white p-4 rounded-lg border border-slate-200 shadow-xs space-y-3">
                    <h4 className="font-sans font-bold text-slate-800 text-xs uppercase tracking-wider">
                      {faseLabels[currentYearEnrollment.faseProcesso].emoji} {faseLabels[currentYearEnrollment.faseProcesso].label}
                    </h4>

                    {currentYearEnrollment.faseProcesso === 'preparo_terra' && (
                    ...
                    {currentYearEnrollment.faseProcesso === 'semeadura' && (
                      <p className="text-xs text-slate-500">
                        Aguardando confirmação dos dados do aluno e responsáveis (Ficha de Dados Gerais).
                      </p>
                    )}

                    {currentYearEnrollment.faseProcesso === 'enraizamento' && (
                    ...
                    {currentYearEnrollment.faseProcesso === 'florescer' && (
                      <ul className="space-y-1.5 text-xs">
                        <li className="flex items-center justify-between">
                    ...
                    {onSaveEnrollment && currentYearEnrollment.faseProcesso !== 'colheita' && (
                      <button
                        onClick={() => {
                          const nextIdx = faseOrderList.indexOf(currentYearEnrollment.faseProcesso) + 1;
